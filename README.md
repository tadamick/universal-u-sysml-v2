...
The work described here is an initial contribution toward that infrastructure: a vendor-neutral, formally specified reference model of a typical university enrollment process, built from publicly available higher education standards documentation — IPEDS, PESC, IMS LIS — and structured using the CoSMA framework expressed in SysML v2. It is modeled directly on the Apollo 11 reference model published by Airbus, which demonstrated that the CoSMA framework could produce a formally rigorous, reproducible artifact from historical documentation alone.

The proposed reference model applies the Purpose, Operational, and Functional layers of CoSMA framework to the enrollment process, stopping deliberately at the Functional layer boundary. This is not a limitation — it is the boundary at which vendor-neutrality is most defensibly preserved and most practically useful. The layers above describe what a university enrollment system must do and who must be able to do it, independent of any specific platform. Below the Functional boundary, the model becomes a configuration mapping problem: a given platform either satisfies a Functional requirement as stated or it does not. That evaluation is demonstrated through a gap analysis instrument that maps Functional requirements against a representative SIS platform, without embedding platform-specific assumptions into the reference model itself.
