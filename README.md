...
The argument of this work is specific: higher education SIS implementations have the structural characteristics that make Model-Based Systems Engineering the appropriate specification approach, and the sector has not yet recognized this. High interface complexity. Multi-vendor delivery. Long asset lifecycles where the specification must outlast any particular implementation. These are the conditions under which formal models — vendor-neutral, machine-readable, formally traceable from mission to requirement to interface to implementation — provide value that current document-driven approaches cannot. They are also, precisely, the conditions of every major university technology deployment.
...
When the wiring harnesses manufactured in Hamburg and those manufactured in Toulouse were brought together for final assembly, there was a mismatch that delayed the program by two years and cost Airbus an estimated €4.8 billion. Two facilities within the same company, using the same CAD platform but running incompatible versions of it, had each implemented their component correctly against their own interpretation of what the interface between those components required. This was not a vendor failure — it was a specification failure. Natural language requirements support multiple valid interpretations. When two teams independently implement against the same ambiguous requirement, they will produce components that each satisfy the document and do not connect to each other. Integration fails not because anyone did their job wrong, but because the specification discipline was not adequate to the coordination problem.
...
The question is not whether MBSE is theoretically applicable to higher education IT - it is. The quesiton is whether the sector will build the infrastructure to support it before the cost of not doing so becomes impossible to ignore.

**The Proposal**

This work proposes neither that higher education abandon traditional project management practices nor that new methods will immediately transform how institutions specify and procure technology. The infrastructure required to operationalize MBSE in higher education IT — vendor intake processes designed to accept formal specifications, practitioner training in [SysML v2](https://www.omg.org/sysml/sysmlv2/), governance frameworks that include formal model validation — does not yet exist at scale. What does exist is the methodology, the tooling, and the prior art from adjacent domains sufficient to build the formal specification layer that higher education technology deployments will eventually require.

The work described here is an initial contribution toward that infrastructure: a vendor-neutral, formally specified reference model of a typical university course registration process, built from publicly available higher education standards documentation — IPEDS, PESC, IMS LIS — and structured using the Complex System Modeling Architecture (CoSMA) framework expressed in SysML v2 language. It is modeled directly on the [Apollo 11 reference model](https://github.com/airbus/apollo-11-sysml-v2) published by Airbus, which demonstrated that the CoSMA framework could produce a formally rigorous, reproducible artifact from historical documentation alone.

The proposed reference model applies the Purpose, Operational, and Functional layers of CoSMA framework to the course registration process, stopping deliberately at the Functional layer boundary. This is not a limitation — it is the boundary at which vendor-neutrality is most defensibly preserved and most practically useful. The layers above describe what a university course registration process must do, independent of any specific platform. Below the Functional boundary, the model becomes a configuration mapping problem: a given platform either satisfies a Functional requirement as stated or it does not. That evaluation is demonstrated through a gap analysis that maps Functional requirements against a representative SIS platform, without embedding platform-specific assumptions into the reference model itself.

**A Working Reference Model**

The Logical and Technical layers of the CoSMA framework — which describe abstract component assignments and platform-specific implementation mappings respectively — are intentionally excluded from the reference model. Those layers assume design freedom that does not exist when deploying a configurable commercial platform: the relevant architecture decisions have largely been made by the vendor. Forcing requirements through Logical and Technical layers in a commercial off the shelf (COTS) implementation adds abstraction overhead without analytical value. The gap analysis serves the purpose those layers would otherwise fulfill, in a form that is more directly usable by implementation teams.

The reference model is designed to be useful at two different levels of institutional maturity. For an institution currently managing an SIS implementation, the Purpose through Functional layers provide a vendor-neutral requirements baseline that can be used immediately — as a statement of work, as an acceptance testing framework, as a gap analysis tool against a specific platform. For an institution building toward long-term MBSE capability, the same model is a first contribution to a growing institutional reference architecture that will eventually cover admissions, financial aid, advising, and the full enrollment lifecycle. The same artifact serves both purposes because the Functional boundary is where immediate utility and long-term durability intersect.
